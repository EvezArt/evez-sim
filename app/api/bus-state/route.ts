...
const CIRCUIT_REGISTRY = [
  ...
  { id:'r189_probe', type:'topo_spider', label:'R189 Probe', description:'N=141=3\u00d747 tau=4 omega_k=2 \u00b7 fire TBD \u00b7 V_est\u22487.261', omega_k:2, N:141, active:true, layer:'spine', truth_state:'pending' },
  ...
  { id:'google_pw_compromised', type:'prime_void', label:'Google PW Compromised', description:'Password compromised as of 2026-02-24 \u00b7 needs manual rotation', N:83, active:true, layer:'auth', truth_state:'pending' },
]

async function readGistState() {
  try {
    const r = await fetch(GIST_URL, { headers:{'User-Agent':'evez-sim/0.1.0'}, next:{revalidate:30} })
    if (!r.ok) return null
    const data = await r.json()
    const files = data.files
    const f = files['hyperloop_state.json'] || files['activity_feed.json']
...
export async function GET() {
  ...
  const V_global = gist_data?.V ?? 7.240012
  const current_round = gist_data?.current_round ?? 188
  const fire_count = gist_data?.fire_count ?? 35
  const spine_length = gist_data?.spine_length ?? 3847
  const circuit_health = 'YELLOW' // Actions block = degraded
  const res = {
    entities: CIRCUIT_REGISTRY, circuit_health, V_global, current_round, fire_count, spine_length,
